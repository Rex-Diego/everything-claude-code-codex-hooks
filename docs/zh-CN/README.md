...
同步脚本会以 add-only 策略合并 ECC MCP 服务器到现有的 `~/.codex/config.toml`，并把 Codex 原生 hook 写入 `~/.codex/hooks.json`，同时保留已有自定义 hook 组。
...
| Hooks | 1 | `.codex/hooks.json` —— 从 `hooks/hooks.json` 生成的 Codex 原生 hook 配置 |
...
当前 Codex 版本支持通过 `hooks.json` 执行原生 hook。ECC 会通过 `scripts/codex/codex-hook-runner.js` 把规范的 `hooks/hooks.json` 适配到 Codex 支持的事件（`SessionStart`、`PreToolUse`、`PermissionRequest`、`PostToolUse`、`UserPromptSubmit`、`Stop`）。

适配器会把 Claude hook 的旧式 pass-through stdout 转换成 Codex-safe JSON：普通放行输出 `{}`，真正阻止 `PreToolUse` 时输出 Codex 可解析的 `permissionDecision: "deny"`，旧式 Stop/PostToolUse 阻止反馈会转成 `decision: "block"`。这修复了实际运行中可能出现的 `invalid pre-tool-use JSON output` 和 `invalid post-tool-use JSON output`。

Codex 当前并不执行 ECC hook 目录里的所有 Claude-only 事件，因此适配器会跳过 `PostToolUseFailure` 和 `SessionEnd` 等不支持的事件。同步脚本运行时会保留 `~/.codex/hooks.json` 中已有的自定义 hook。
...
| **钩子事件** | 8 种类型 | 15 种类型 | 原生 `hooks.json` 子集 | 11 种类型 |
| **钩子脚本** | 20+ 个脚本 | 16 个脚本 (DRY 适配器) | ECC Codex 适配器 | 插件钩子 |
...
| **秘密检测** | 基于钩子 | beforeSubmitPrompt 钩子 | Hook + 沙箱 | 基于钩子 |
| **自动格式化** | PostToolUse 钩子 | afterFileEdit 钩子 | Stop/PostToolUse hook | file.edited 钩子 |
...
* Codex 使用原生 `hooks.json`；ECC 适配器复用共享的 Node hook 脚本，并跳过不支持的 Claude-only 事件
